		...
		if (timeDiff > 0) {
			...
			for (PowerHost host : this.<PowerHost> getHostList()) {
				...
				try {
					...
					file = new BufferedWriter(new FileWriter("/home/jquintana/Desktop/cpuConsumo2.txt", true));
					// verificación de host inicial
					if (host.getId() == 0) {
						// Agregar encabezado del archivo
						file.append("tiempo,host,cpuP,cpuA\n");
					}
					// Agregamos data al archivo
					file.append(currentTime + "," + host.getId() + "," + (previousUtilizationOfCpu * 100) + "," + (utilizationOfCpu * 100) + "\n");
				...
				Log.formatLine(
						...
						Writer output = null;
						try {
							output = new BufferedWriter(new FileWriter("/home/jquintana/Desktop/energiaConsumo.txt", true));
							if (host.getId() == 0) {
								output.append("tiempo,host,consumoEnergia  W*sec\n");
							}
							output.append(currentTime + "," + host.getId() + "," + timeFrameHostEnergy + "\n");
						} catch (IOException e1) {
			...
			Writer outputN = null;
			try {
				outputN = new BufferedWriter(new FileWriter("/home/jquintana/Desktop/energiaConsumoDataCenter.csv", true));
							// if (host.getId() == 0) {
							// 	output.append("tiempo,consumoEnergia  W*sec\n");
							// }
				outputN.append(currentTime + ";" + timeFrameDatacenterEnergy + "\n");
						} catch (IOException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						} finally {
